...
class DashboardThiduaT08Controller {
    ...
    async createManualListLTTSL(req, res) {
            ...
            if (kpiList && kpiList.length > 0) {
                try {
                    ...
                    for (const object of kpiList) {
                        ...
                        const existingKpi = await sequelize.query(
                            `SELECT * from sl_thidua_ltt_082025 WHERE issue_date =  to_date(:date,'dd-mm-rrrr')
                                    and province = :province
                                    `,
                            {
                                replacements: { date: info.date, province: info.province },
                        ...
                        if (existingKpi.length > 0) {
                            await sequelize.query(
                                `delete from sl_thidua_ltt_082025 
                                            WHERE  issue_date = to_date(:date,'dd-mm-rrrr')
                                        and province = :province
                                            `,
                                {
                            ...
                            await sequelize.query(
                                `insert into  sl_thidua_ltt_082025 
                                    (issue_date, province, amount)
                                    values
                                    (
                                    to_date(:date,'dd-mm-rrrr'),
    ...
    async createManualListCameraSL(req, res) {
            ...
            if (kpiList && kpiList.length > 0) {
                try {
                    ...
                    for (const object of kpiList) {
                        ...
                        const existingKpi = await sequelize.query(
                            `SELECT * from sl_thidua_camera_082025 WHERE issue_date =  to_date(:date,'dd-mm-rrrr')
                                    and province = :province
                                    `,
                            {
                                replacements: { date: info.date, province: info.province },
                        ...
                        if (existingKpi.length > 0) {
                            await sequelize.query(
                                `delete from sl_thidua_camera_082025 
                                            WHERE  issue_date = to_date(:date,'dd-mm-rrrr')
                                        and province = :province
                                            `,
                                {
                            ...
                            await sequelize.query(
                                `insert into  sl_thidua_camera_082025 
                                    (issue_date, province, amount)
                                    values
                                    (
                                    to_date(:date,'dd-mm-rrrr'),
    ...
    getSoLuongLTTDetail(req, res) {
        const date = req.query.date;
        if (date) {
            const query = `select province, issue_date, amount   from sl_thidua_ltt_082025
            where issue_date =  to_date(:date,'dd/mm/rrrr')
            order by  issue_date, province  `;
            sequelize.query(query, {
                replacements: { date: date },
    ...
    getSoLuongCameraDetail(req, res) {
        const date = req.query.date;
        if (date) {
            const query = `select province, issue_date, amount   from sl_thidua_camera_082025
            where issue_date =  to_date(:date,'dd/mm/rrrr')
            order by  issue_date, province  `;
            sequelize.query(query, {
                replacements: { date: date },
